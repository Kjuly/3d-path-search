...
{
	unsigned key = Hash( _state );	// 将State映射为键值
...
int MicroPather::Solve( void* startNode, void* endNode, vector< void* >* path, float* cost )
{
	...
	*cost = 0.0f;
	...
	while ( !open.Empty() )
	{
		PathNode* node = open.Pop();	// 从open队列中获取节点
		...
		{
			closed.Add( node );	// 将当前节点置为已访问过节点

			// We have not reached the goal - add the neighbors.
			GetNodeNeighbors( node, &nodeCostVec );	// 获取当前节点的相邻节点
			...
			for( int i=0; i<node->numAdjacent; ++i )
			{
				...
				PathNode * child = nodeCostVec[i].node; // 新子节点(当前节点相邻中的一个)
				float newCost = node->costFromStart + nodeCostVec[i].cost; // 新节点的代价

				PathNode * inOpen   = child->inOpen ? child : 0;	// 该子节点是否已在open表中
				PathNode * inClosed = child->inClosed ? child : 0;	//				   closed
				//if( child->inOpen ) printf("Child->inOpen\n");
				//if( child->inClosed ) printf("Child->inClosed\n");
				// 在open和closed两个表中其中一个
				PathNode * inEither = (PathNode*)( ((MP_UPTR)inOpen) | ((MP_UPTR)inClosed) );

				MPASSERT( inEither != node );			// 处理与父节点相同的节点
				MPASSERT( !( inOpen && inClosed ) );	// 处理即在open表又在closed表中的节点

				//-----------------------------------------
				// 在open和closed两个表中其中一个
				if ( inEither ) {
					//printf("%f---%f\n",newCost,child->costFromStart);
					if ( newCost < child->costFromStart ) {
						//printf("啦啦啦啦啦啦啦啦啦啦啦啦啦啦啦啦啦啦啦啦啦啦\n");
						child->parent = node;
